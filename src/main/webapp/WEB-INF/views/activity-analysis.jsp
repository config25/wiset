...
(function () {
  ...
  top3.forEach(function(p){
    ...
    p.items.forEach(function(it,i){ var vcol = p.tone==='blue' ? 'var(--blue)' : 'var(--amber)';
      html += '<div class="row items-center gap-14"><span class="mono fw-700" style="font-size:12px;color:var(--ink-400);width:18px;">'+(i+1)+'</span>'+chip(it.ic,p.tone,42,21,11)+'<div style="flex:1;"><div class="row items-center justify-between mb-6"><span class="fw-700" style="font-size:14px;">'+esc(it.n)+'</span><span class="fw-800" style="font-size:15px;color:'+vcol+';">'+it.v+(it.gap?'<span class="caption" style="color:var(--pink);margin-left:4px;">'+esc(it.gap)+'</span>':'')+'</span></div><div class="score-track"><div class="score-fill '+p.barCls+'" style="width:'+it.v+'%;"></div></div></div></div>';
    });
    html += '</div></div>';
  });
  html += '</div>';
  ...
  fetch(ctx + '/api/activity-analysis/report' + (did?('?diagnosisId='+encodeURIComponent(did)):'')).then(function(r){ return r.ok ? r.json() : null; }).then(function(rep){
    ...
    // 점수/근거/CFI/요약/JD 는 관계형 테이블에서 온 값을 우선 적용. 비면 위 content/목업 유지.
    if(rep){
