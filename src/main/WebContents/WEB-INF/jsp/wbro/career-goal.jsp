    ...
    <div style="max-width:880px; margin:16px auto 0;">
      ...
      <div class="card" style="padding:32px;">
        ...
        <div class="row gap-24">
          <div class="flex-1">
            ...
            <select id="industry" class="gsel">
              ...
              <option>생명 및 자연과학 관련직</option><option>교육 관련직</option><option>보건의료 관련직</option><option>디자인/방송 관련직</option><option>운전/운송 관련직</option><option>건축/토목 공학 관련직</option><option>기계 관련직</option><option>재료 관련직</option><option>화학/식품가공 관련직</option><option>전기/전자 관련직</option><option>정보통신 관련직</option><option>환경/에너지/안전 관련직</option><option>기술영업/판매 관련직</option><option>기타</option>
            </select>
          </div>
          <div class="flex-1">
            <div class="row items-center gap-8" style="margin-bottom:14px;">
            ...
            <select id="job" class="gsel">
              <option value="" selected>선택</option>
              <option>연구개발직</option><option>기술직</option><option>연구지원직</option>
            </select>
          </div>
        </div>
        ...
        <div style="margin-top:28px;">
          ...
          <%-- 스크랩 전부 표시하되 ~3개 높이로 고정 + 넘치면 스크롤(나머지도 선택 가능) --%>
          <div class="col gap-10" id="targetList" style="max-height:264px; overflow-y:auto; padding-right:4px;"></div>
...
(function () {
  ...
  $('nextBtn').addEventListener('click', function(){
    // 희망 업종·직무는 필수(미선택 시 다음 단계 차단) — type1 역량평가의 target_role 기준
    var indVal = $('industry').value, jobVal = $('job').value;
    if (!indVal || !jobVal) {
      alert('희망 업종과 희망 직무를 선택해주세요.');
      (!indVal ? $('industry') : $('job')).focus();
      return;
    }
    // DB 저장 없이 선택값만 로컬(sessionStorage) 보관 → 최종 단계(분석 시작)에서 일괄 저장
    var data = {
      industry: indVal,   // 희망 업종
      job: jobVal,        // 희망 직무
