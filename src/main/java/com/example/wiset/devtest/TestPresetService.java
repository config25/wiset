...
public class TestPresetService {
    ...
    /** 현 상황(user 1) 전량 삭제 + 자기소개서 비우기. (브라우저 세션 wb_* 은 클라이언트가 삭제) */
    @Transactional
    ...
    /** user 1 현 상황 요약(항목 건수 + 최종학력 식별자) — 테스트 콘솔 상태 뷰어용. */
    public Map<String, Object> state() throws Exception {
        Map<String, Object> m = new LinkedHashMap<>();
        List<EducationDto> edu = cs.listEducation();
        m.put("education", edu.size());
        String identity = null;
        for (EducationDto e : edu) {
            if (e.isFinal()) { identity = join(e.getSchoolName(), e.getMajorName()); break; }
        }
        if (identity == null && !edu.isEmpty()) identity = join(edu.get(0).getSchoolName(), edu.get(0).getMajorName());
        m.put("identity", identity);
        m.put("career", cs.listCareer().size());
        m.put("research", cs.listResearch().size());
        m.put("activity", cs.listActivity().size());
        m.put("training", cs.listTraining().size());
        m.put("certificate", cs.listCertificate().size());
        m.put("award", cs.listAward().size());
        m.put("overseas", cs.listOverseas().size());
        m.put("language", cs.listLanguage().size());
        Object urls = cs.getPortfolio().get("urls");
        m.put("portfolio", (urls instanceof List) ? ((List<?>) urls).size() : 0);
        Object ct = cs.getCover().get("title");
        m.put("coverTitle", (ct == null || ct.toString().trim().isEmpty()) ? null : ct);
        return m;
    }
