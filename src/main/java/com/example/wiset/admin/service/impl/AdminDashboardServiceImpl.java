...
/**
 * 17_관리자 통합 대시보드 조립 (조회 전용) — 레거시 wbridge commonDAO idiom 이식판(로컬 실행).
 ...
 *   SQL 은 sqlmap/mngr/adminDashboard_mapper.xml (namespace mngr.adminDashboard) 참조.
 *
 *   [wbridge 포팅 델타] @Service("adminDashboardService") extends DefaultServiceImpl (패키지 wbridge.mngr...),
...
public class AdminDashboardServiceImpl {
    ...
    public AdminDashboardServiceImpl(CommonDAO commonDAO) {
    ...
    // ---------------------------------------------------------------- realtime
    private Map<String, Object> buildRealtime() throws Exception {
        Map<String, Object> r = commonDAO.selectOne("mngr.adminDashboard.realtimeKpi");
    ...
    // ----------------------------------------------------------------- persona (DB)
    private List<Map<String, Object>> buildPersona() throws Exception {
        List<Map<String, Object>> rows = commonDAO.selectList("mngr.adminDashboard.personaInflow");
        long total = 0;
    ...
    // ------------------------------------------------------------------ funnel (DB)
    private Map<String, Object> buildFunnel() throws Exception {
        Map<String, Object> c = commonDAO.selectOne("mngr.adminDashboard.funnelCounts");
    ...
    // ------------------------------------------------------------------ answer
    private Map<String, Object> buildAnswer() throws Exception {
        Map<String, Object> out = new LinkedHashMap<>();

        // [DB] 좋아요/싫어요 = 만족도 별점 ≥4.0 / ≤2.0
        Map<String, Object> th = commonDAO.selectOne("mngr.adminDashboard.thumbs");
        ...
        for (Map<String, Object> row : commonDAO.<Map<String, Object>>selectList("mngr.adminDashboard.ratingDist")) {
    ...
    // ----------------------------------------------------------------- quality (DB · 자동 평가)
    private List<Map<String, Object>> buildQuality() throws Exception {
        Map<String, Object> q = commonDAO.selectOne("mngr.adminDashboard.quality");
        List<Map<String, Object>> list = new ArrayList<>();
    ...
    // ------------------------------------------------------------------ system
    private Map<String, Object> buildSystem() throws Exception {
        Map<String, Object> out = new LinkedHashMap<>();
        ...
        for (Map<String, Object> row : commonDAO.<Map<String, Object>>selectList("mngr.adminDashboard.recentBatches")) {
            Map<String, Object> m = new LinkedHashMap<>();
            m.put("id", row.get("id"));
